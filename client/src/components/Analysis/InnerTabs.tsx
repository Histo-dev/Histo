...
type TabKey = 'overview' | 'topN' | 'detail'
...
export default function InnerTabs({ selected = 'overview', onSelect }: Props) {
  return (
    <div className={styles.container} role="tablist" aria-label="내부 탭">
      <button
        className={`${styles.tab} ${selected === 'overview' ? styles.active : ''}`}
      ...
      <button
        className={`${styles.tab} ${selected === 'detail' ? styles.active : ''}`}
        onClick={() => onSelect?.('detail')}
        role="tab"
        aria-selected={selected === 'detail'}
      >
        상세
      </button>
    </div>
