...
const categorize = (domain: string, title?: string, categoryMap?: Record<string, string>): string => {
...
const aggregateAndStore = async () => {
  const { sessions = [], visits = [], categoryMap = {} } = await storageGet<{
    sessions: Session[]
    visits: Visit[]
  ...
  if (!Array.isArray(sessions) || sessions.length === 0) {
    await storageSet({
      siteStats: {},
      categoryStats: {},
      dailyTotals: { date: DAY_KEY(), totalMinutes: 0, totalSites: 0, totalVisits: visits.length } satisfies DailyTotal,
      analysisState: 'idle',
